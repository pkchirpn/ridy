...
export class FleetNewComponent {
  ...
  async submitForm() {
    ...
    const result:any = await firstValueFrom(this.createRiderGQL.mutate({ input: { phoneNumber: `${phoneNumberPrefix.substring(1)}${phoneNumber}`, mobileNumber: `${mobileNumberPrefix.substring(1)}${mobileNumber}`, ..._formValue } }));
    if((result.errors?.length ?? 0) > 0) {
      this.messageService.error((result.errors ?? []).map((error :any)=> error.message).join(','));
    } else {
