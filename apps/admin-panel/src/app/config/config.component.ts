...
export class ConfigComponent implements OnInit, OnDestroy {
  ...
  async next() {
    switch (this.currentStep) {
      case (0): {
        ...
        const result:any = await firstValueFrom(this.updatePurchaseGql.mutate({ code: this.currentConfig.currentConfiguration.purchaseCode ?? '' }));
        console.log(result);
        if (result.data?.updatePurchaseCode.status == UpdatePurchaseCodeStatus.ClientFound) {
          this.clients = result.data?.updatePurchaseCode.clients?.map((client:any )=> client.ip) ?? [];
      ...
      case (1): {
        ...
        const result:any = await firstValueFrom(this.updateMapsGql.mutate({ backend: this.currentConfig.currentConfiguration.backendMapsAPIKey, adminPanel: this.currentConfig.currentConfiguration.adminPanelAPIKey }));
  ...
  async done() {
    ...
    const result:any = await firstValueFrom(this.updateFirebaseGql.mutate({ keyFileName: this.currentConfig.currentConfiguration.firebaseProjectPrivateKey }));
